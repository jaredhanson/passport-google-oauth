...
function Strategy(options, verify) {
  ...
  this._userURL = options.userURL || 'https://www.googleapis.com/oauth2/v2/userinfo';
  this._userParser = options.userParser || function(body, done){
    try {
...
Strategy.prototype.userProfile = function(accessToken, done) {
  var strategy = this;
  this._oauth2.get(this._userURL, accessToken, function (err, body, res) {
    if (err) { return done(new InternalOAuthError('failed to fetch user profile', err)); }

    strategy._userParser(body, done);
  });
}
